...
| `fix_kind` | str | For `found` rows: **how** the vulnerable versions left the lockfile at `fix_commit_sha`. `upgraded` — the dependency still resolves at non-vulnerable versions; `removed` — the dependency is gone from every parsed root lockfile (this conflates a direct dependency deliberately dropped with a transitive dependency no longer pulled in because a parent was upgraded); `anomalous_still_present` — a vulnerable version still resolves in a lockfile outside the search set (flagged, not silently misclassified); `unknown` — backfill cache miss in `--cache-only` mode. Null for non-`found` rows and parquets mined before the column existed (backfill with `scripts/backfill_fix_kind.py`). |
